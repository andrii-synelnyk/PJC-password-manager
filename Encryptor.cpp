...
int Encryptor::getCheckSumFromFile(const std::string& data){
...
bool Encryptor::isValidDecryption(const std::string& data, int storedChecksum) {
    return computeChecksum(data) == storedChecksum;
}

void Encryptor::shiftFile(const std::string& filePath, const std::string& password, bool decrypt) {
    ...
    if (decrypt) {
        ...
        try {
            ...
            std::cout << "Cannot encrypt already encrypted file";
            return;
        }catch (const std::exception& ex){}
    }
    ...
    if (decrypt) {
        if (isValidDecryption(tempData, storedChecksum)) {
            ...
            std::cout << "Wrong password. Unable to decrypt.";
            return;
        }
    }
    else {
        ...
        originalData += "\n" + std::to_string(checksum); // append checksum for decryption validity check
    }
